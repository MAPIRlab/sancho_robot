...
enum Level { LEVEL_OFF, LEVEL_LOW, LEVEL_MEDLOW, LEVEL_MED, LEVEL_MEDHIGH, LEVEL_HIGH, LEVEL_FULL };
Level current_level = LEVEL_OFF;

enum SystemMode { IDLE_MODE, LISTENING_MODE, THINKING_MODE, SPEAKING_MODE };
...
void animationThinking() {
  ...
  index += dir;
  if (index == NUM_PIXELS - 1 || index == 0) dir *= -1;
  //delay(50);
}

// Called periodically during eye animation.
void user_loop(String incomingData) {
  if (incomingData.length() > 0) {
    Serial.println("[MOUTH] Received: " + incomingData + "0");
    ...
    // Color change only in SPEAKING_MODE
      ...
      } else if (incomingData == "full") {
        current_level = LEVEL_FULL;
      } else {
        Serial.println("[MOUTH] Unknown command0");
        return;
      }
      lightLEDs(LED_MASKS[current_level], current_color);
    }
