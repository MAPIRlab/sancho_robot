...
: Node("cliff_detector")
{
  ...
  image_sub_ = image_transport::create_camera_subscription(
    this, "/camera/depth/image_raw", std::bind(&CliffDetectorNode::depthCb, this, _1, _2), "raw");
  ...
  const sensor_msgs::msg::CameraInfo::ConstSharedPtr & info)
{
  try {
    ...
    RCLCPP_INFO(this->get_logger(), "Running cliff detection.");
    auto msg = detector_.detectCliff(image, info);
    RCLCPP_INFO_ONCE(this->get_logger(), "Publishing detected cliff points.");
    pub_points_->publish(msg);

    // Publishes debug depth image with obstacles
    if (detector_.getPublishDepthEnable()) {
      RCLCPP_INFO_ONCE(this->get_logger(), "Publishing debug depth image with detected cliffs.");
      pub_.publish(detector_.getDebugDepthImage());
    }
    RCLCPP_INFO(this->get_logger(), "Cliff detection completed.");
