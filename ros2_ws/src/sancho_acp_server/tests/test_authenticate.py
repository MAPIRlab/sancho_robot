"""Test ACP authenticate operation.

Verifies that the agent correctly handles authenticate requests.
"""
...
async def main():
        ...
        init_resp = await asyncio.wait_for(
            conn.initialize(
                ...
                client_info=Implementation(
                    name="auth-test", title="Auth Test", version="0.1.0"
                ),
            ),
            timeout=10.0,
        )
        assert init_resp.agent_info is not None
        print(f"[TEST] ✅ Initialize OK — agent: {init_resp.agent_info.name}")
    ...
    # Test: authenticate
    print("[TEST] Sending authenticate (method_id='basic')...")
    try:
    ...
    if errors:
        ...
        print("\n[TEST] ✅ All authenticate tests passed!")
        sys.exit(0)
