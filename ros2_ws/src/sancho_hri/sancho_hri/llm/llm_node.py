...
from sancho_interfaces.srv import GetModels, SanchoPrompt, Prompt, Embedding, LLMLoadModel, LLMUnloadModel, GetActiveModels, SetActiveModel
...
class LLMNode(Node):
    ...
    PROVIDER_CLASS_MAP = {
        PROVIDER.OPENAI: ("sancho_hri.llm.providers.openai_provider", "OpenAIProvider"),
        PROVIDER.MISTRAL: ("sancho_hri.llm.providers.mistral_provider", "MistralProvider"),
        PROVIDER.LLAMA: ("sancho_hri.llm.providers.llama_provider", "LLaMaProvider"),
        PROVIDER.PHI: ("sancho_hri.llm.providers.phi_provider", "PhiProvider"),
        PROVIDER.QWEN: ("sancho_hri.llm.providers.qwen_provider", "QwenProvider"),
        PROVIDER.DEEPSEEK: ("sancho_hri.llm.providers.deepseek_provider", "DeepSeekProvider"),
        PROVIDER.GEMINI: ("sancho_hri.llm.providers.gemini_provider", "GeminiProvider"),
        PROVIDER.SBERT: ("sancho_hri.llm.providers.sbert_provider", "SBERTProvider"),
        PROVIDER.E5: ("sancho_hri.llm.providers.e5_provider", "E5Provider"),
        PROVIDER.BAAI: ("sancho_hri.llm.providers.baai_provider", "BAAIProvider"),
        PROVIDER.GEMMA: ("sancho_hri.llm.providers.gemma_provider", "GemmaProvider"),
        PROVIDER.FALCON: ("sancho_hri.llm.providers.falcon_provider", "FalconProvider"),
        PROVIDER.YI: ("sancho_hri.llm.providers.yi_provider", "YIProvider"),
        PROVIDER.MAPIRBOT: ("sancho_hri.llm.providers.mapirbot_provider", "MapirbotProvider"),
    }
    ...
    def __init__(self):
        ...
        self.raw_prompt_srv = self.create_service(Prompt, 'sancho_hri/llm/raw_prompt', self.handle_raw_prompt)
    ...
    def handle_prompt(self, request, response):
        ...
        return response
    def handle_raw_prompt(self, request, response):
        """Technical prompt interface used by LLMEngine.

        Unlike handle_prompt (SanchoPrompt), this handler exposes the full
        provider/model/prompt_system/messages_json API so internal components
        (classifiers, generators, memory manager) can control all LLM parameters.
        """
        self.get_logger().info(
            f"📖 Raw prompt for provider='{request.provider}', model='{request.model}'"
        )
        try:
            ...
            self.get_logger().info(f"✅ Raw prompt done using provider='{provider_name}', model='{model_used}'")
        except Exception as e:
            self._fill_response(response, False, str(e))
            self.get_logger().error(f"❌ Raw prompt failed: {str(e)}")
    ...
    def handle_load_model(self, request, response):
        ...
        for item in request.items:
            result = LLMLoadUnloadResult(provider=item.provider)
            try:
