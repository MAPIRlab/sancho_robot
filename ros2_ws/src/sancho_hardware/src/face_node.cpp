    ...
    FaceNode() : Node("face_node")
    {
        ...
        if (serial_fd_ != -1)
        {
            ...
            else
            {
                close(serial_fd_);
                serial_fd_ = -1;
                RCLCPP_ERROR(get_logger(), "Error configurando el puerto %s", path.c_str());
            }
        }
        else
        {
             RCLCPP_WARN(get_logger(), "No se encontro /dev/esp32. El nodo funcionara SIN hardware (Modo Simulacion).");
        }
