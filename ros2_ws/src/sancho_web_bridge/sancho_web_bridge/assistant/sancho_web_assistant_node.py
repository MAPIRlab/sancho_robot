...
from sancho_interfaces.msg import R2WMessage
from sancho_interfaces.msg import FaceprintEvent
from sancho_interfaces.srv import SanchoPrompt
from sancho_interfaces.srv import STT, TTS


class SanchoWebNode(Node):

    def __init__(self):
        super().__init__("sancho_web_bridge.assistant")
...
class SanchoWeb:
    ...
    def on_web_message(self, key, msg) -> JSONMessage:
        ...
        if type == MessageType.PROMPT:
            ...
            value, method, intent, arguments, provider, model = self.sancho_prompt_request(prompt.value, prompt.chat_id)
            ...
            if transcription: # Si hay transcripcion
                value, method, intent, arguments, provider, model = self.sancho_prompt_request(transcription, audio_prompt.chat_id)
...
def main(args=None):
    ...
    sancho_web_bridge.assistant = SanchoWeb()

    sancho_web_bridge.assistant.spin()
    rclpy.shutdown()