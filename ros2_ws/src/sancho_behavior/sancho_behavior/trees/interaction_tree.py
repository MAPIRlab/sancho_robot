...
from sancho_behavior.behaviors.interaction import SetFaceMode, ListenToUser, GenerateLLMResponse, RespondUser

def create_interaction_subtree() -> py_trees.behaviour.Behaviour:
    """Creates the branch that manages active face tracking."""
    interaction_seq = py_trees.composites.Sequence(name="Interaction", memory=False)
    ...
    conversation.add_children([
        ...
        ListenToUser(),             # Fails if timeout/silence -> drops out of conversation
    ...
    handle_interaction_end.add_children([interaction_tasks, SetFaceMode("idle"), reset_engaged])
