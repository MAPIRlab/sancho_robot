...
from sancho_interfaces.msg import ModelSpeaker,ModelItem, SpeechLoadUnloadResult
from sancho_interfaces.srv import TTS, TTSGetActiveModel, TTSGetModels, TTSSetActiveModel, SpeechLoadModel, SpeechUnloadModel
...
class TTSNode(Node):
    ...
    MODELS_CLASS_MAP = {
        TTS_MODELS.BARK: ("sancho_hri.speech.tts.bark_tts", "BarkTTS"),
        TTS_MODELS.CSS10: ("sancho_hri.speech.tts.css10_tts", "CSS10TTS"),
        TTS_MODELS.GOOGLE: ("sancho_hri.speech.tts.google_tts", "GoogleTTS"),
        TTS_MODELS.PIPER: ("sancho_hri.speech.tts.piper_tts", "PiperTTS"),
    ...
    def handle_load_model(self, request, response):
        ...
        for item in request.items:
            result = SpeechLoadUnloadResult(model=item.model)
            try:
    ...
    def handle_unload_model(self, request, response):
        ...
        for model_name in request.models:
            result = SpeechLoadUnloadResult(model=model_name)
