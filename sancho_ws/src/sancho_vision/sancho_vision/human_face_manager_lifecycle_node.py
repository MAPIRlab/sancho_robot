...
class HumanFaceManager:
    ...
    def spin(self):
        ...
        if self.node.face_timeout_response:
            ...
            if self.gui_request_sent_info:
                tid = self.gui_request_sent_info[-1]
                self.training_request(
                    String(data="cancel"),
                    String(data=json.dumps({"tid": tid}))
                )
            self.gui_request_sent_info = None
    ...
    def process_recognitions(self, msg):
        for det, recog in zip(msg.detections, msg.recognitions):
            ...
            tid = det.tid  # NUEVO: Extraemos el TID del tracking
            face_updated = recog.face_updated
            face_aligned = self.node.bridge.imgmsg_to_cv2(recog.face_aligned, "bgr8")

            # NUEVO: Ignorar los mensajes de estado que usamos para evitar el spam
            IGNORE_STATES = ['Analyzing...', 'Waiting for input...', 'Pending...', 'Waiting confirmation...']
            if classified_name in IGNORE_STATES:
                continue
            ...
            if distance < self.LOWER_BOUND:  # Desconocido (< 0.70) → pedir nombre
                classified_id = None
            ...
            elif distance < self.UPPER_BOUND:  # Dudoso (< 0.90) → pedir confirmación siempre
                if score >= self.DETECTOR_BOUND and self.ask_unknowns:
                    if not self.gui_request_sent_info:
                        face_aligned_base64 = self.node.bridge.cv2_to_base64(face_aligned, max_dim=480)
                        if self.gui_request("ask_if_name", json.dumps({"image": face_aligned_base64, "name": classified_name})):
                            ...
                            self.node.get_logger().info("Error al enviar petición de confirmar nombre a la GUI")

            else:  # Reconoce con seguridad (>= 0.90)
    ...
    def process_face_name_response(self, name):
        if not self.gui_request_sent_info:
            self.node.get_logger().warn("Received name response, but no GUI request is active.")
            return

        # 1. NO QUITAMOS EL CANDADO AÚN. Extraemos los datos.
        ...
        # 2. VACIAR LA COLA DE FRAMES ATRASADOS (Evita procesar el pasado)
        while not self.node.face_recognitions_queue.empty():
            self.node.face_recognitions_queue.get()

        # 3. AHORA SÍ, QUITAMOS EL CANDADO.
        self.gui_request_sent_info = None
    ...
    def process_face_question_response(self, answer):
        ...
        if answer:
            ...
            # VACIAR LA COLA DE FRAMES ATRASADOS
            while not self.node.face_recognitions_queue.empty():
                self.node.face_recognitions_queue.get()
            ...
            # Si dice que NO es él, pedimos el nombre en la GUI.
            # Mantenemos el candado puesto porque lanzamos otra ventana.
            if self.gui_request("get_name", json.dumps({"image": face_aligned_base64})):
                self.gui_request_sent_info = [classified_id, classified_name, face_aligned_base64, features, score, distance, tid]
                self.ask_user(QUESTION.GET_NAME)
