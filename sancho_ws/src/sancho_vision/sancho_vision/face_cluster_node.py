...
class Track2D:
    def __init__(self, track_id, init_pos_xy, person_id=0, max_misses=60, dt=0.1):
        # Estado [x, y, vx, vy]; medida [x, y]
        self.id = track_id
        ...
        # Covarianzas
        self.kf.P *= 1.0
        # Ruido de proceso (blanco discreto) para 2D con block_size=2 → 4x4
        ...
        self.kf.x[:2] = np.array(init_pos_xy, dtype=float).reshape((2, 1))
        self.misses = 0
    ...
    def update(self, meas_pos_xy, person_id=0):
        z = np.array(meas_pos_xy, dtype=float).reshape((2,))
        self.kf.update(z)
        self.misses = 0
        if person_id > 0:
            self.person_id = person_id
...
class SORT2D:
    def __init__(self, max_misses=30, dist_threshold=100, dt=0.1, identity_penalty=1.0):
        self.tracks = []
        self.next_id = 1
        self.max_misses = max_misses
    ...
    def update(self, detections):
        # detections: list of (pos_xy, person_id)
        # Predict all tracks
        ...
        # Asignación por identity primero
        ...
        if N > 0 and M > 0:
            ...
            for idx_i, i in enumerate(unmatched_tracks):
                ...
                for idx_j, j in enumerate(unmatched_dets):
                    ...
                    base_cost = euclidean2(tr.pos, det_pos)
        ...
        # Misses y supervivientes
        survivors = []
        for i, tr in enumerate(self.tracks):
            if i in assigned_tr or not tr.miss():
                survivors.append(tr)
            else:
                print(f"[SORT2D] Track lost: id={tr.id}, person_id={tr.person_id}, last_pos={tr.pos}")
        self.tracks = survivors

        # Nuevos tracks para detecciones sin asignar
        for j, (det_pos, pid) in enumerate(detections):
            if j not in assigned_det:
                self.tracks.append(Track2D(self.next_id, det_pos, pid, self.max_misses, self.dt))
                self.next_id += 1
...
class FaceClusterServiceNode(Node):
    def __init__(self):
        ...
        self.tracker = SORT2D(max_misses=max_misses,
    ...
    def face_cb(self, msg: FaceRecognitionArray):
        ...
        for det, recog in zip(msg.detections, msg.recognitions):
            ...
            # Calcula Z solo para proyectar X,Y y luego OLVIDARLA
            Z = (self.k / det.height)/10  # se usa solo para la proyección
            u, v = det.corner.x - det.width/2, det.corner.y - det.height/2  # centro aproximado de la caja
            ...
            try:
                ...
                p = pt_head.point
                # Solo XY para el tracker
                detections.append(([p.x, p.y], pid))

                if pid > 0:
                    self.get_logger().info(f"Face detected: ID={pid}, Distance={recog.distance:.3f} position XY: ({round(X,2)}, {round(Y,2)})")
            except Exception as e:
                self.get_logger().warning(f"Transform failed: {str(e)}")
                continue

        self.get_logger().info(f"\n-------Actualizando tracks (2D)-----------------------------\n")
        self.tracker.update(detections)
        self.get_logger().info(f"Lista de (track_id, person_id) en los tracks: {[(tr.id, tr.person_id) for tr in self.tracker.tracks]}")

        # Publica marcadores (z fijo)
        ma = MarkerArray()
        for tr in self.tracker.tracks:
            # Esfera
            m = Marker(header=Header(frame_id=self.head_frame, stamp=msg.header.stamp),
                       ns='face_sphere', type=Marker.SPHERE, action=Marker.ADD)
            m.lifetime = Duration(seconds=(0.1)).to_msg()
            m.id = tr.id
            m.pose.position.x = float(tr.pos[0])
            m.pose.position.y = float(tr.pos[1])
            m.pose.position.z = float(1.0)  # Z fija para visualización
            m.scale.x = m.scale.y = m.scale.z = 0.5
            m.color.a = 1.0; m.color.r = 0.0; m.color.g = 1.0; m.color.b = 0.0
            ma.markers.append(m)

            # Texto
            text_m = Marker(header=Header(frame_id=self.head_frame, stamp=msg.header.stamp),
                            ns='face_text', type=Marker.TEXT_VIEW_FACING, action=Marker.ADD)
            text_m.lifetime = Duration(seconds=(2)).to_msg()
            text_m.id = tr.id
            text_m.pose.position.x = float(tr.pos[0])
            text_m.pose.position.y = float(tr.pos[1])
    ...
    def compute_cluster_cb(self, request, response):
        # Clustering en 2D
        pts = np.array([tr.pos for tr in self.tracker.tracks])  # Nx2
        ...
        for lbl in set(labels):
            ...
            centroid = cluster_pts.mean(axis=0)  # [cx, cy]
            dist = np.linalg.norm(centroid)     # distancia en XY
            if dist < best_dist:
        ...
        if best_lbl is not None:
            ...
            # Devuelve XY; Z la fijamos a 0.0 ya que no se usa
            response.centroid_x = float(best_centroid[0])
            response.centroid_y = float(best_centroid[1])
            response.centroid_z = 0.0
        else:
            response.member_ids = []
        return response
