...
class LLMTaskAI(TaskAI): # Only callable tasks as a single SanchoPrompt, no tasks like classification or memory builder
    ...
    @classmethod
    def get_name(cls, message: str, args: dict) -> tuple[dict[str, Any], str, str]:
        return cls._execute(ExtractNamePrompt(message), required=["name_said", "name"], default={"name_said": False, "name": ""})

    @classmethod
    def confirm_name(cls, message: str, args_: dict) -> tuple[dict[str, Any], str, str]:
        return cls._execute(ConfirmNamePrompt(message), required=["answer_said", "answer"], default={"answer_said": False, "answer": ""})

    @classmethod
    def no_one_known(cls, message: str, args: dict) -> tuple[dict[str, Any], str, str]:
        return cls._execute(NoOneKnownPrompt(), required=["response"], default={"response": ""})

    @classmethod
    def some_known(cls, message: str, args: dict) -> tuple[dict[str, Any], str, str]:
        return cls._execute(SomeKnownPrompt(args.get("people", "")), required=["response"], default={"response": ""})

    @classmethod
    def all_known(cls, message: str, args: dict) -> tuple[dict[str, Any], str, str]:
        return cls._execute(AllKnownPrompt(args.get("people", "")), required=["response"], default={"response": ""})
